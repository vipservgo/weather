...
const API_KEY = "ee7c4b79648c7ec65f4c16b0b11a0ffe"; // API key
    ...
    if(index === 0) {
        return `<div class="mt-3 d-flex justify-content-between">
                    <div>
                        ...
                        <h6 class="my-3 mt-3">ტემპ: ${((weatherItem.main.temp - 273.15).toFixed(2))}°C</h6>
                        <h6 class="my-3">ქარი: ${weatherItem.wind.speed} M/S</h6>
                        <h6 class="my-3">ტენიანობა: ${weatherItem.main.humidity}%</h6>
                    </div>
        ...
        return `<div class="col mb-3">
                    <div class="card border-0 bg-secondary text-white">
                        <div class="card-body p-3 text-white">
                            ...
                            <h6 class="card-text my-3 mt-3">ტემპ: ${((weatherItem.main.temp - 273.15).toFixed(2))}°C</h6>
                            <h6 class="card-text my-3">ქარი: ${weatherItem.wind.speed} M/S</h6>
                            <h6 class="card-text my-3">ტენიანობა: ${weatherItem.main.humidity}%</h6>
                        </div>
                    </div>
                </div>`;
    }
...
const getCityCoordinates = () => {
    ...
    fetch(API_URL).then(response => response.json()).then(data => {
        if (!data.length) return alert(`ქალაქი ვერ მოიძებნა ${cityName}`);
